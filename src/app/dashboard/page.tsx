...
//       if (userFetchAttempted) return; // Prevent any re-entry

//       setUserFetchAttempted(true); // Set immediately
...
//     fetchData();
//   }, [dispatch, userFetchAttempted]);

//   // A separate useEffect for dispatching remaining data fetches
//   useEffect(() => {
//     if (user) {
...
import type { ExpenseEntry, IncomeEntry } from "@/types/Interface"; // added for clarity and better type handling
...
export default function DashboardPage() {
  ...
  // Dashboard no longer needs to handle authentication
  // ProtectedRoute handles all authentication logic
  // Just wait for user data to be available
  useEffect(() => {
    if (user) {
      setUserLoading(false);
    }
  ...
  // Second fetch: all user-related data
  useEffect(() => {
    if (user) {
      dispatch(fetchBalanceThunk());
      dispatch(fetchIncomeStatsThunk());
...
// 🧩 Extracted SummaryCard for reuse
